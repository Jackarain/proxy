...
const option k_options[] = {
	...
	{ "tun", option_kind::boolean, "启用 TUN 设备模式（TUN2SOCKS，仅 Linux，需指定 proxy_pass）。", false, false, 0, "", {}, false, true, true,
	  "启用 TUN 设备模式：创建 TUN 设备抓取 IP 数据包，按 proxy_domains / proxy_cidr 分流，命中走上游代理，未命中直连。系统路由需外部脚本配置。修改后需重启实例。", "TUN" },
	{ "tun_name", option_kind::string, "TUN 设备名称，留空自动分配。", false, false, 0, "", {}, false, true, false, "", "TUN" },
	{ "tun_mtu", option_kind::integer, "TUN 设备 MTU，默认 1500。", true, false, 1500, "", {}, false, true, false, "", "TUN" },
	{ "proxy_domains", option_kind::string_list, "代理域名后缀列表（命中走上游代理，可重复添加）。", false, false, 0, "", {}, false, true, false,
	  "代理域名后缀列表（可添加多条），后缀匹配：填 google.com 命中 www.google.com。命中域名的解析经上游 DoH，数据面走上游代理；未命中本地解析并直连。", "TUN" },
	{ "proxy_cidr", option_kind::string_list, "代理 CIDR 列表（命中走上游代理，IPv4/IPv6，可重复添加）。", false, false, 0, "", {}, false, true, false,
	  "代理 CIDR 列表（可添加多条），如 0.0.0.0/8、2001:db8::/32。命中走上游代理，未命中直连目标。", "TUN" },
