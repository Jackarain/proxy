...
RUN cd /proxy/test/lnx && g++ -std=c++20 test_dataflow.cpp -o test_dataflow -ldl -pthread && g++ -std=c++20 test_dataflow_ws.cpp -o test_dataflow_ws -ldl -pthread && g++ -std=c++20 test_dataflow_ws_manager.cpp -o test_dataflow_ws_manager -ldl -pthread
RUN cd /root && mkdir -p lib && cp /proxy/build/lib/libdata_flow.so /root/lib/ && cp /proxy/build/third_party/boost/libs/url/libboost_url.so.1.86.0 /root/lib/ && cp /proxy/test/lnx/test_dataflow /proxy/test/lnx/test_dataflow_ws /proxy/test/lnx/test_dataflow_ws_manager /root/ && cp /proxy/test/flow.conf /root/
