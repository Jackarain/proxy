...
- `GET /api/instances/:id/status`（实时报告 + 各用户累计上下行）、`GET .../logs?since=N`（日志快照/增量）
- `POST /api/instances/:id/usage/reset`（重置累计上传/下载：`{"user":"..."}` 单个用户，空体或空 user 全部用户）
...
- 每实例独立状态（`store/app.ts` 的 `perInst`）：当前页签、配置草稿与搜索词、
  ...
  实例互不串扰。
- 累计流量：实例上报的 `rx_bytes`/`tx_bytes` 为会话级累计值（重启归零），
  launcher 按用户折算增量后随实例持久化，并经 `GET .../status` 的 `usage` 字段下发；
  状态页「累计上传/累计下载」及汇总随之跨实例重启、页面刷新与 launcher 重启延续。
  支持按用户或整体重置（`POST .../usage/reset`），重置仅清零展示口径，配额累计不变。
