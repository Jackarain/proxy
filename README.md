...
| server_listen | 监听的地址以及端口，格式为 `addr:port`，支持 `v6only`(如: `addr:port-v6only`)，支持同时监听在多个地址上，亦可监听在 unix domain socket 上，格式：`unix://path/to/proxy.sock`|
| stdio | 用于作为 [proxytunnel](https://github.com/proxytunnel/proxytunnel) 代理隧道功能时，向外发起连接的主机和端口，格式为：`host:port` |
...
| ssl_certificate_dir | `ssl` 证书密钥等存放目录，`proxy_server` 会自动查找这个目录下的子目录，每个目录必须为一个域名相关的证书文件 (如果证书密钥文件加密，则需要配置 password.txt 用于指定密码)，证书若过期将自动每5分钟重新热加载一次证书可避免服务重启 |
