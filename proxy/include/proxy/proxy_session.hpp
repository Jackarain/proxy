...
namespace proxy {
	...
	struct proxy_server_option
	{
		...
		// - 启动后每 5 秒建 1 条连接，直到池满该数量；连接被取走或断开后
		//   立即异步补充，始终维持该数量的空闲连接。
		// - 0 表示不启用预选连接池；默认 20。
		size_t proxy_pass_pool_size_{ 20 };
	...
	private:
		...
		// 协议侦测协程.
		template <typename T>
		net::awaitable<void> proto_detect(bool handshake_before = true) noexcept;
		...
		net::awaitable<void> on_http_json_impl(const http_context& hctx) noexcept;
