...
class _ConfigEditPageState extends State<ConfigEditPage> {
  ...
  late final TextEditingController _dnsCacheSize = TextEditingController(
    text: c.dnsCacheSize.toString(),
  );
  late final TextEditingController _dnsCacheTtl = TextEditingController(
    text: c.dnsCacheTtl.toString(),
  );
  ...
  void _finishSave(List<String> domains, List<String> cidrs) {
    ...
    final vpn =
        c
          ...
          ..dnsCache = _dnsCache
          ..dnsCacheSize = int.tryParse(_dnsCacheSize.text.trim()) ?? 200
          ..dnsCacheTtl = int.tryParse(_dnsCacheTtl.text.trim()) ?? 60
          ..noIpv6 = _noIpv6
  ...
  Widget _buildDnsTab(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        TextField(
          controller: _dnsCacheSize,
          keyboardType: TextInputType.number,
          decoration: const InputDecoration(
            labelText: '缓存条数',
            helperText: 'LRU 上限, 超出后淘汰最久未访问, 0 表示禁用缓存',
            border: OutlineInputBorder(borderRadius: BorderRadius.zero),
          ),
        ),
        const SizedBox(height: 12),
