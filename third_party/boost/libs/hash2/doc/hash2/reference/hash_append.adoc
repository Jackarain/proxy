...
* If `T` is an instance of `std::optional`, calls `hash_append(h, f, v.has_value())`, then, if `v` is engaged, calls `hash_append(h, f, *v)`;
* If `T` is an instance of `std::variant`, if `v` is not valueless, calls `hash_append_size(h, f, v.index())` and `hash_append(h, f, w)` where `w` is the active alternative of `v`;
* If `T` is `std::monostate`, calls `hash_append(h, f, '\x00')`;
* Otherwise, the result is a compile-time error.

Remarks: ::
  In case the above description would result in no calls being made (e.g. for a range of constant size zero, a valueless `std::variant`, or a described `struct` with no bases and members),
...
Effects: ::
  Equivalent to `hash_append_range(h, f, first, last); hash_append_size(h, f, m);`, where `m` is `std::distance(first, last)`.
