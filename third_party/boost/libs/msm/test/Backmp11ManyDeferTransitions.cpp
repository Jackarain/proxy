...
struct Action
{
    ...
    void operator()(const Event&, Fsm& fsm, Ts...)
    {
        if constexpr (std::is_same_v<Event, Event1>)
        {
            fsm.processed_events.push_back(1);
        }
        if constexpr (std::is_same_v<Event, Event2>)
        {
            fsm.processed_events.push_back(2);
        }
        if constexpr (std::is_same_v<Event, Event3>)
        {
            fsm.processed_events.push_back(3);
        }
        if constexpr (std::is_same_v<Event, Event4>)
        {
            fsm.processed_events.push_back(4);
        }
    }
};
...
struct Sm1_ : state_machine_def<Sm1_>
{
    ...
    using transition_table = mp11::mp_list<
        ...
        Row < State1, Event4, State2, Action >,

        Row < State2, Event2, State3, Action >,

        Row < State3, Event1, State4, Action >,
        Row < State3, Event3, State5, Action >,

        Row < State4, Event3, State5, Action >,

        Row < State5, Event1, State4, Action >
    >;

    std::vector<size_t> processed_events;
};
...
using TestMachines = mp11::mp_list<
    ...
    StateMachine<Sm1_>,
    StateMachine<Sm1_, favor_compile_time_config>
...
BOOST_AUTO_TEST_CASE_TEMPLATE(uml_defer_transitions, TestMachine, TestMachines)
{
    ...
    // Queue: Event2
    BOOST_REQUIRE(state_machine.get_pending_events().size() == 1);
    
    state_machine.process_event(Event3());
    // Queue: Event2, Event3
    BOOST_REQUIRE(state_machine.get_pending_events().size() == 2);
    
    state_machine.process_event(Event1());
    // Queue: Event2, Event3, Event1
    BOOST_REQUIRE(state_machine.get_pending_events().size() == 3);
    
    state_machine.process_event(Event4());
    // Event4 gets consumed, new state is State2.
    // Event2 gets consumed, new state is State3 (queue: Event3, Event1)
    // Event3 gets consumed, new state is State5 (queue: Event1)
    // Event1 gets consumed, new state is State4
    BOOST_REQUIRE(state_machine.get_pending_events().size() == 0);
    std::vector<size_t>& processed_events = state_machine.processed_events;
    BOOST_REQUIRE(state_machine.template is_state_active<State4>());
    std::vector<size_t> expected_processed_events{4, 2, 3, 1};
    BOOST_REQUIRE(processed_events == expected_processed_events);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(action_defer_transitions, TestMachine, TestMachines)
{
    ...
    state_machine.process_event(Event1());
    // Queue: Event1, Event3, Event2
    BOOST_REQUIRE(state_machine.get_pending_events().size() == 3);
    
    state_machine.process_event(Event4());
    // Event4 gets consumed, new state is State2.
    // Event1 gets deferred (queue: Event3, Event2, Event1)
    ...
    std::vector<size_t> expected_processed_events{4, 2, 1, 3};
    BOOST_REQUIRE(processed_events == expected_processed_events);
}
