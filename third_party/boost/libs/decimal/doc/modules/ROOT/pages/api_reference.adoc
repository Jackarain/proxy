...
| xref:config.adoc#configuration_user[`BOOST_DECIMAL_ALLOW_IMPLICIT_INTEGER_CONVERSIONS`] | xref:config.adoc#host_device[`BOOST_DECIMAL_HOST_DEVICE`]
| xref:config.adoc#configuration_user[`BOOST_DECIMAL_ALLOW_IMPLICIT_FLOAT_CONVERSIONS`] | xref:config.adoc#cuda_constexpr[`BOOST_DECIMAL_CUDA_CONSTEXPR`]
...
| xref:config.adoc#enable_cuda[`BOOST_DECIMAL_ENABLE_CUDA`] |
|===
