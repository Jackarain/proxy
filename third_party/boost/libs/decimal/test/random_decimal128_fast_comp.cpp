...
void random_LT(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal_fast128_t dec1 {val1};
        const decimal_fast128_t dec2 {val2};
    ...
    // Edge cases
    BOOST_TEST(decimal_fast128_t(dist(rng)) < std::numeric_limits<decimal_fast128_t>::infinity());
    BOOST_TEST(!(decimal_fast128_t(dist(rng)) < -std::numeric_limits<decimal_fast128_t>::infinity()));
    BOOST_TEST(!(decimal_fast128_t(dist(rng)) < std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
    BOOST_TEST(!(std::numeric_limits<decimal_fast128_t>::quiet_NaN() < std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
}
...
void random_mixed_LT(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal_fast128_t dec1 {val1};
        const T dec2 {static_cast<T>(decimal_fast128_t(val2))};
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T dec1 {static_cast<T>(decimal_fast128_t(val1))};
        const decimal_fast128_t dec2 {val2};
    ...
    // Edge Cases
    BOOST_TEST_EQ(decimal_fast128_t(1) < T(1), false);
    BOOST_TEST_EQ(decimal_fast128_t(10) < T(10), false);
    BOOST_TEST_EQ(T(1) < decimal_fast128_t(1), false);
    BOOST_TEST_EQ(T(10) < decimal_fast128_t(10), false);
...
void random_mixed_LE(T lower, T upper)
{
    ...
    BOOST_TEST(dist(rng) <= std::numeric_limits<decimal_fast128_t>::infinity());
    BOOST_TEST(!(dist(rng) <= -std::numeric_limits<decimal_fast128_t>::infinity()));
    BOOST_TEST(!(dist(rng) <= std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
}
...
void random_GT(T lower, T upper)
{
    ...
    const auto inf = std::numeric_limits<decimal_fast128_t>::infinity();
    BOOST_TEST(!(decimal_fast128_t(dist(rng)) > inf));
    BOOST_TEST((decimal_fast128_t(dist(rng)) > -inf));
    BOOST_TEST(!(decimal_fast128_t(dist(rng)) > std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
    BOOST_TEST(!(std::numeric_limits<decimal_fast128_t>::quiet_NaN() > std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
}
...
void random_EQ(T lower, T upper)
{
    ...
    BOOST_TEST(!(std::numeric_limits<decimal_fast128_t>::quiet_NaN() == std::numeric_limits<decimal_fast128_t>::quiet_NaN()));
    BOOST_TEST(decimal_fast128_t{0} == -decimal_fast128_t{0});
}
...
void random_mixed_EQ(T lower, T upper)
{
    ...
    // Edge Cases
    BOOST_TEST_EQ(decimal_fast128_t(1), T(1));
    BOOST_TEST_EQ(decimal_fast128_t(10), T(10));
    BOOST_TEST_EQ(decimal_fast128_t(100), T(100));
    BOOST_TEST_EQ(decimal_fast128_t(1000), T(1000));
    BOOST_TEST_EQ(decimal_fast128_t(10000), T(10000));
    BOOST_TEST_EQ(decimal_fast128_t(100000), T(100000));
    BOOST_TEST_EQ(BOOST_DECIMAL_DEC_NAN == T(1), false);
    BOOST_TEST_EQ(BOOST_DECIMAL_DEC_INFINITY == T(1), false);
}
...
void random_SPACESHIP(T lower, T upper)
{
    ...
    BOOST_TEST((decimal_fast128_t(dist(rng)) <=> std::numeric_limits<decimal_fast128_t>::quiet_NaN()) == std::partial_ordering::unordered);
    BOOST_TEST((std::numeric_limits<decimal_fast128_t>::quiet_NaN() <=> std::numeric_limits<decimal_fast128_t>::quiet_NaN()) == std::partial_ordering::unordered);
}
...
void random_mixed_SPACESHIP(T lower, T upper)
{
    ...
    if (!BOOST_TEST((dist(rng) <=> std::numeric_limits<decimal_fast128_t>::quiet_NaN()) == std::partial_ordering::unordered))
    {
        // LCOV_EXCL_START
        const auto eval {dist(rng) <=> std::numeric_limits<decimal_fast128_t>::quiet_NaN()};
...
int main()
{
    ...
    constexpr auto pos_zero = boost::decimal::decimal_fast128_t{0U, 0, false};
    constexpr auto neg_zero = boost::decimal::decimal_fast128_t{0U, 0, true};
