...
^1^ The one edge case where allocation may occur is if you are parsing a string to an 80 or 128-bit `long double` or `__float128`, and the string is over 1024 bytes long.
