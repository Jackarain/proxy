...
void ip_packet::begin_ipv4(const net::ip::address_v4 &src,
    const net::ip::address_v4 &dst, uint8_t ttl)
{
    builder_begin(4);
...
void ip_packet::begin_ipv6(const net::ip::address_v6 &src,
    const net::ip::address_v6 &dst, uint8_t hop_limit)
{
    builder_begin(6);
    const auto s = src.to_bytes();
    const auto d = dst.to_bytes();
...
void ip_packet::begin_tcp(uint16_t src_port, uint16_t dst_port, uint32_t seq,
    uint32_t ack, uint8_t flags, uint16_t window, const void *options,
    size_t options_len)
{
    if (bld_.version == 0) {
...
void ip_packet::parse_transport(uint8_t protocol, const uint8_t *seg,
    size_t seg_len)
{
    transport_parsed_ = false;
    switch (protocol) {
