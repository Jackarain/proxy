...
bool buffer_accountant::reserve(size_t n)
{
...
void buffer_accountant::release(size_t n)
{
    used -= n;
}
...
net::ip::tcp::endpoint tcp_flow::remote_endpoint() const noexcept
{
    if (key.family == 6)
    {
    ...
    const ip_packet_info& ip, const uint8_t* payload, size_t len)
{
    ...
    // 校验 TCP 校验和
    if (tcp_udp_checksum(
            ip.family, ip.src_ip, ip.dst_ip, IPPROTO_TCP_V, payload, len) != 0)
    {
        stats_.rx_dropped.fetch_add(1, std::memory_order_relaxed);
        return;
    }
    ...
    const five_tuple key = make_five_tuple(ip.src_ip,
        ip.dst_ip,
        th.src_port,
        th.dst_port,
        IPPROTO_TCP_V,
...
void tcp_engine::flush_reads(tcp_flow& f)
{
    if (f.active_read && f.rx_bytes > 0)
    {
        ...
        for (auto& buf : op.buffers)
        {
            ...
            const size_t take = std::min(buf.size(), n - copied);
            std::memcpy(
                buf.data(), f.rx_data.data() + f.rx_head + copied, take);
            copied += take;
        }
    ...
    if (f.rx_bytes == 0 && f.fin_received && f.active_read)
    {
        auto op = std::move(*f.active_read);
        f.active_read.reset();
...
packet_buffer tcp_engine::build_segment(tcp_flow& f,
    uint32_t seq,
    uint8_t flags,
    ...
    bool with_mss)
{
    ...
    th->window = htons(static_cast<uint16_t>(scaled > 65535 ? 65535 : scaled));
    th->checksum = 0;
    th->urgent = 0;
    ...
    th->checksum = htons(tcp_udp_checksum(family,
        f.key.dst_ip.data(),
        f.key.src_ip.data(),
        IPPROTO_TCP_V,
        base + ip_hdr_len,
...
void tcp_engine::send_segment(tcp_flow& f,
    uint32_t seq,
    uint8_t flags,
    const uint8_t* payload,
    size_t len,
...
net::awaitable<void> tcp_engine::write_loop(tcp_flow_ptr f)
{
    ...
    {
        while (
            flow.active_write && flow.state != tcp_state::CLOSED && !flow.rst)
        {
            // 写数据末尾序号：FIN 发送后 snd_nxt 会再前进 1（FIN 占序号），
            // 写完成只要求对端确认到数据末尾，不等待 FIN 的确认。
            const uint32_t write_end = flow.active_write->start_seq +
            ...
            rto_timer.expires_after(rto);
            auto result = co_await (flow.write_ch->async_receive(
                                        net::as_tuple(net::use_awaitable)) ||
                rto_timer.async_wait(net::as_tuple(net::use_awaitable)));
            if (result.index() == 1 &&
                std::get<0>(std::get<1>(result)) == boost::system::error_code{})
            {
                if (flow.snd_una == last_una)
                {
...
void tcp_engine::retransmit_unacked(tcp_flow& f, size_t max_bytes)
{
    ...
    while (bi < op.buffers.size() && base + op.buffers[bi].size() <= off)
    {
    ...
    while (bi < op.buffers.size() && off < end && budget > 0)
    {
        ...
        send_segment(f,
            op.start_seq + static_cast<uint32_t>(off),
            TCP_ACK | TCP_PSH,
            payload,
            want,
            false);
...
{
    const uint32_t queued = static_cast<uint32_t>(f.rx_bytes + f.ooo_bytes);
...
void tcp_engine::close_flow(tcp_flow& f, const boost::system::error_code& err)
{
    ...
    if (f.pending_accept)
    {
        ...
        const auto it = std::find(
            pending_flows_.begin(), pending_flows_.end(), f.shared_from_this());
    ...
    if (f.state != tcp_state::SYN_RCVD && f.state != tcp_state::SYN_ACK_SENT)
...
// ---- tcp_flow 外部回调（tun_tcp_socket 接口）----
...
net::ip::tcp::endpoint tcp_flow::original_destination() const noexcept
{
    if (key.family == 6)
    {
        net::ip::address_v6::bytes_type b{};
