...
struct tcp_flow : public std::enable_shared_from_this<tcp_flow>
{
    ...
    uint32_t peer_wnd =
        0; // 客户端通告的接收窗口（按对端 scale 放大后的实际值）
    ...
    uint32_t last_wnd_advertised =
        0; // 最近通告的接收窗口（未缩放），窗口恢复检测
    bool fin_sent = false;
...
using tcp_accept_handler =
    net::any_completion_handler<void(boost::system::error_code, tcp_flow_ptr)>;
using tcp_flow_map =
    boost::unordered_flat_map<five_tuple, tcp_flow_ptr, std::hash<five_tuple>>;

template <typename Handler>
void tcp_flow_start_read(tcp_flow_ptr flow,
    mutable_buffer_sequence buffers,
    size_t total,
    Handler handler);
template <typename Handler>
void tcp_flow_start_write(tcp_flow_ptr flow,
...
private:
    ...
    template <typename Handler>
    friend void tcp_flow_start_read(
        tcp_flow_ptr, mutable_buffer_sequence, size_t, Handler);
    template <typename Handler>
    ...
    Handler handler)
{
    ...
    net::dispatch(strand,
            ...
            handler = std::move(handler)]() mutable
        {
            ...
            {
                handler(
                    boost::system::error_code(net::error::connection_reset), 0);
                return;
            }
            ...
            if (flow.active_read)
            {
                ...
                handler(
                    boost::system::error_code(net::error::no_buffer_space), 0);
                return;
            }

            flow.active_read = tcp_flow::read_op{
    ...
    Handler handler)
{
    ...
    net::dispatch(strand,
            ...
            strand]() mutable
        {
            ...
            flow.active_write = tcp_flow::write_op{std::move(buffers),
                total,
                0,
                0,
                0,
