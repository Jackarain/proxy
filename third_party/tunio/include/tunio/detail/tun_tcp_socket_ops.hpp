...
void tun_tcp_socket::do_read_some(
    MutableBufferSequence&& buffers, Handler handler)
{
    ...
    detail::tcp_flow_start_read(
        std::move(flow), std::move(seq), total, std::move(handler));
}

template <typename ConstBufferSequence, typename Handler>
void tun_tcp_socket::do_write_some(
