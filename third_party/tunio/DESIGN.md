...
本引擎在 TCP 处理上采取轻量级转发策略：接收方向缓存乱序段并按序交付，以最低开销完成 L3/L4 拦截；发送方向提供 RTO 重传与零窗口持久探测，写完成回调提前后未确认数据由流级尾部 RTO 接管，保证数据在确认前不丢失。
...
- **重叠写拒绝**：上一写操作未完成时新写入以 `no_buffer_space` 立即
  完成（应用违反串行写规则时的背压兜底）。
- **发到设备即写完成**：写操作在数据全部交给 TUN 设备后完成回调（而非
  等对端 ACK），避免小段触发对端 delayed ACK（约 40ms）串行拖慢吞吐；
  未确认数据由流级尾部重传兜底，与直连/代理端写语义一致。
- **流级尾部重传**：写完成回调时若仍有未确认数据（如末尾段丢失或窗口
  受限下未确认的在途数据），引擎将其拷贝进连接级 `tail_buf`（用户缓冲
  可能已被应用复用，不能继续引用）并由独立 RTO 定时器重传，ACK 部分
  推进时复位退避，全部确认后释放；重传次数超过 `tcp_rto_max_retransmits`
  （默认 8 次）判定发送超时，以 RST 快速释放连接，避免永久悬挂。
- **RTO 重传**：发送协程对本次写操作的未确认范围维护 RTO 计时器
  （默认初始 200ms，指数退避，上限 60s），ACK 无进展时重读用户写缓冲
  重传；更早写操作的未确认尾部由上述流级尾部重传负责，两条 RTO 各自
  重传不重叠的范围。
...
4. **极低的协议开销**：接收方向缓存乱序段并按序交付，缺失段补齐后批量确认，避免人为乱序触发对端快速重传；发送方向以 RTO 计时器重传——写进行中重读用户缓冲，写完成后仅对未确认尾部做窗口上限的有界拷贝并由流级 RTO 重传，实现低 CPU 占用的高速转发。
